...
if [ -d "/usr/local/cuda" ]; then
    echo "  CUDA: $(cat /usr/local/cuda/version.txt 2>/dev/null || nvcc --version | grep release | awk '{print $5}' | tr -d ',')"
else
...
# TensorRT
if [ "${USE_TRT}" = "ON" ]; then
    if dpkg -l | grep -q tensorrt 2>/dev/null || [ -f "/usr/include/aarch64-linux-gnu/NvInfer.h" ] || [ -f "/usr/include/NvInfer.h" ]; then
...
# Build (use nproc for parallel compilation, limit to 2 on Nano to avoid OOM)
NPROC=$(nproc)
if [ "$NPROC" -gt 2 ]; then
    NPROC=2  # Jetson Nano has limited RAM, avoid OOM with too many parallel jobs
fi
