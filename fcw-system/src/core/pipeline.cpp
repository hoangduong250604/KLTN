...
bool Pipeline::processFrame() {
    ...
    if (config_.fixedEgoSpeedKmh >= 0.0f) {
        ...
        if (!gotOxts) {
            // Real-world camera/video: no OXTS sensor — use fixed 50 km/h
    ...
    if (evalEnabled_ && evalLog_.is_open()) {
        // Map class IDs to names (BDD100K - matching labels.txt order)
        ...
        if (activeTracks.empty()) {
            // No detections this frame - still log frame info
